...
show_figures_clumps=False             # True
...
# smoothRadius=10000 ; 
threshold_clumpiness=3 ; deblend_clumpmap=1 # (dovrebbe essere 1 da threshold 3 in su) - se vuoi fare il deblending della segmentation map dei clumps (Per identificare i NUCLEI !!!)
    ...
    for jkk in np.arange(len(bande_all)) :
      ...
      if use_statmorph==True : 
        ...
        try : 
          ...
          ellipticity_SM.append(np.round(morph.ellipticity,3))
          elongation_SM.append(np.round(morph.elongation,3))
...
# -----------------------------------------------------------------------------------------------------------
  ...
  #########################################################################
  gini_SM=np.array(gini_SM)
  flag_SM=np.array(flag_SM)
  sn_per_pixel_SM=np.array(sn_per_pixel_SM)
  asymmetry_SM=np.array(asymmetry_SM)
  concentration_SM=np.array(concentration_SM)
  smoothness_SM=np.array(smoothness_SM)
  m20_SM=np.array(m20_SM)
  ellipticity_SM=np.array(ellipticity_SM)
  elongation_SM=np.array(elongation_SM)
  rpetro_SM=np.array(rpetro_SM)
  rhalf_SM=np.array(rhalf_SM)
  ...
  #quit()
  galaxy_table=np.hstack((IDvector,photoband_vector,clumpvector,clumpvector2,Rmax_vector,gini_vector,m20_vector,concentration_vector,asymmetry_vector1,shapeasy_vector,smoothness_vector,sn_per_pixel_vector1,sn_per_pixel_vector2,area_vector,flag_SM,gini_SM,m20_SM,concentration_SM,asymmetry_SM,smoothness_SM,sn_per_pixel_SM,rpetro_SM)) # ,ginim20a_vector,distlotz_vector)) #,nc_nodeblvec,nclumpvector))
  ...
  inputlist=np.array(['# IDgal','band','c1','c2','Rmax','gini','m20','C','A1','shapeasy','S','SNpixel1','SNpixel2','Area','SM_flag','SM_gini','SM_m20','SM_C','SM_A','SM_S','SM_SNpixel','SM_Rpetro\n'])   #   # VEDI A INIZIO PROGRAMMA
